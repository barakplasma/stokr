window.Stokr.Controller = (function () {
  ...
  return {
    // todo - On app init, check if we already saved the ui state, if we do, populate the ui state
    // in Model with that data
    init: function () {
    ...
    // todo refactor reOrderStocks to fix this with dynamic data
    // maybe use elm.appendChild(elm.removeChild
    reOrderStocks: function (stockToMove, upOrDownCount) {
      console.log(stockToMove,upOrDownCount);
      // processModel()
      //   .then(currentStocks => {
    ...
    // todo - Applying Filters and reloading the browser should return to the same filtered view
    // with the filters filled in
